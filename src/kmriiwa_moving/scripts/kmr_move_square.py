...
def move_square():
    ...
    while not rospy.is_shutdown():
        ...
        rospy.sleep(2.0)  # Move forward for 2 seconds (adjust as needed)

        # Stop the robot
        move_cmd.linear.x = 0.0
        move_cmd.angular.z = 0.0
        cmd_vel_pub.publish(move_cmd)
        rospy.sleep(1.0)  # Pause for 1 second (adjust as needed)

        # Rotate the robot by a specified angle (90 degrees for a square)
        move_cmd.linear.x = 0.0
        move_cmd.angular.z = angular_velocity
        cmd_vel_pub.publish(move_cmd)
        rospy.sleep(1.57)  # Rotate for 90 degrees (adjust as needed)
