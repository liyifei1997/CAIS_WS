...
class BaseArmSequencer:
    ...
    def run(self):
        ...
        # 3) arm 归位
        rospy.loginfo("Safe pose")
        self.go_pose_via_trajectory_deg([-120, 60, 0, -60, 0, 60, 0], duration_s=2.0, wait=True)
        ...
        if self.wait_traj_server(3.0):
            ...
            for pose_deg, t in zip([p1,p2,safe],[t1,t2,t3]):
                pt = JointTrajectoryPoint()
                pt.positions = [deg2rad(v) for v in pose_deg]
            ...
            self.go_pose_via_trajectory_deg([90,90,0, 90,0,0,0], 2.0, True)
            self.go_pose_via_trajectory_deg([-120, 60, 0, -60, 0, 60, 0], 2.0, True)
